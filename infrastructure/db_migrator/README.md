...
- Python 3.10+
- PostgreSQL
- Зависимости: `alembic`, `sqlalchemy`, `sqlmodel`, `psycopg[binary]>=3.1`

```bash
pip install alembic sqlalchemy sqlmodel "psycopg[binary]>=3.1"
```
...
success = run_migration(db_url="postgresql+psycopg://user:pass@localhost:5432/dbname", schema_path="./models.py")
```
...
```bash
python -m db_migrator.reset postgresql+psycopg://user:pass@localhost:5432/dbname
```
